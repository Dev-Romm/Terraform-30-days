...
| `min_size` | `number` | Minimum number of EC2 instances in the Auto Scaling Group | N/A | Yes |
| `max_size` | `number` | Maximum number of EC2 instances in the Auto Scaling Group | N/A | Yes |
| `instance_type` | `string` | EC2 instance type (e.g., t3.micro, t3.small) | `"t3.micro"` | No |
...
|------|-------------|-------|
| `alb_dns_name` | The DNS name of the Application Load Balancer | Use this to access your webserver cluster |
...
| `alb_arn` | The ARN of the Application Load Balancer | Use in other AWS resources that reference the ALB |
| `target_group_arn` | The ARN of the target group | Reference for custom target group rules or monitoring |
...
output "website_url" {
  value = "http://${module.webserver_cluster.alb_dns_name}:${var.server_port}"
}
```
...
3. **Application Load Balancer (ALB)** - Distributes traffic across running instances
4. **Target Group** - Groups of instances to receive traffic; includes health checks
5. **ALB Listener** - Routes HTTP traffic on your specified port
...
6. **Default VPC only** - This module uses the default VPC and subnets. For custom VPC support, add `vpc_id` and `subnet_id` variables.
